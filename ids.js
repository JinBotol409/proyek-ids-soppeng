...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/dashboard/change_password.php") return handleChangePassword();
...
async function handleDashboard(request, env) {
  const user = await env.DB.prepare("SELECT * FROM tamu ORDER BY id LIMIT 1").first();
...
async function handleMyPosts(request, env) {
  const user = await env.DB.prepare("SELECT * FROM tamu ORDER BY id LIMIT 1").first();
  const posts = await env.DB.prepare(`SELECT * FROM repair_posts WHERE user_id=? ORDER BY id DESC`).bind(user?.id || 0).all();
