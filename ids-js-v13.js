...
const PUBLIC_FILE_ROUTE = "/idsrepair";
...
export default {
  async fetch(request, env) {
    ...
    try {
      if (path.startsWith("/idsrepair/")) return handleBucketFile(request, env);
...
function mediaCloudUrl(value, folder) {
  ...
  if (!value) return "";
  if (value.startsWith(PUBLIC_FILE_ROUTE + "/")) return value;
  if (/^https?:\/\//i.test(value)) {
    try {
      const u = new URL(value);
      return bucketPublicUrl(u.pathname.replace(/^\/+/, ""));
    } catch (e) {
      return "";
    }
  }
  const clean = value.replace(/^\/+/, "");
  if (clean.startsWith(folder + "/")) return bucketPublicUrl(clean);
...
function r2KeyFromPublicPath(url = "") {
  ...
  if (!value) return "";
  if (value.startsWith(PUBLIC_FILE_ROUTE + "/")) {
    return value.slice((PUBLIC_FILE_ROUTE + "/").length).replace(/^\/+/, "");
  }
  try {
    const u = new URL(value);
...
async function handleBucketFile(request, env) {
  const url = new URL(request.url);
  let key = decodeURIComponent(url.pathname.replace(/^\/idsrepair\/?/, "")).replace(/^\/+/, "");
  ...
  if (key.startsWith("ecu_files/")) {
    ...
    return new Response("Folder idsrepair tidak diizinkan", { status: 403 });
  }
  ...
  headers.set("Cache-Control", key.startsWith("ecu_files/") ? "private, max-age=0, no-store" : "public, max-age=31536000, immutable");
...
<img src="/idsrepair/css/ecu-logo.png" alt="IDS ecu Repair Soppeng">
</div>
...
<img src="/idsrepair/gallery/banner-trusted-ai-ready.png" alt="IDS ECU Repair teruji terpercaya SEO Google dan AI Agent Ready" loading="lazy" class="trust-banner-img">
</div>
</section>
...
<img src="/idsrepair/gallery/ecu1.jpg" loading="lazy" alt="IDS Repair">
<img src="/idsrepair/gallery/ecu2.jpg" loading="lazy" alt="IDS Repair">
<img src="/idsrepair/gallery/ecu3.jpg" loading="lazy" alt="IDS Repair">
<video controls><source src="/idsrepair/gallery/video1.mp4" type="video/mp4"></video>
</div>
</div>
...
<link rel="icon" href="/idsrepair/css/ecu-logo.png" type="image/png">
...
<img src="/idsrepair/fighter/awaluddin-soppeng.png" alt="Awaludin Supernew">
...
<div class="container">
<img src="/idsrepair/css/og-banner-read-write.png" class="banner">
...
<div class="container">
<section class="hero">
<img src="/idsrepair/css/og-remap-prosedur-order.png" class="banner" alt="Prosedur Remap ECU IDS Repair">
