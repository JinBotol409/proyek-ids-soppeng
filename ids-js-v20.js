...
export default {
  async fetch(request, env) {
    ...
    try {
      if (path.startsWith("/media/")) return handleMediaFile(request, env);
      ...
      if (path === "/posts/delete_repair.php" || path === "/posts/delete_repair/" || path === "/posts/delete_repair") return handleDeleteRepair(request, env);
      ...
      if ((path === "/remap/" || path === "/remap") && url.searchParams.has("duitku_callback")) return handleDuitkuCallback(request, env);
      if (path === "/remap/duitku-callback" || path === "/remap/callback") return handleDuitkuCallback(request, env);
      if (path === "/remap/download.php" || path === "/remap/download" || path === "/remap/download/") return handleRemapDownload(request, env);
      if (path === "/remap/" || path === "/remap") return handleRemap(request, env);
...
function bucketPublicUrl(key = "") {
  ...
  if (!clean) return "";
  return `${PUBLIC_FILE_ROUTE}/${clean}`;
}
...
function mediaCloudUrl(value, folder) {
  ...
  // Kalau sudah memakai route Worker baru, biarkan.
  if (clean.startsWith(PUBLIC_FILE_ROUTE + "/")) return clean;

  // Normalisasi data lama dari database: CDN lama, /uploads lama, /css lama.
  ...
  clean = clean
    .replace(/^uploads\/repairs\//i, "repairs/")
    .replace(/^uploads\/videos\//i, "videos/")
    .replace(/^uploads\/ecu_files\//i, "ecu_files/")
    .replace(/^uploads\/profile\//i, "profile/")
    .replace(/^uploads\/gallery\//i, "gallery/")
    .replace(/^uploads\/og_images\//i, "og_images/")
    .replace(/^uploads\//i, "")
    .replace(/^css\//i, "css/")
    .replace(/^profile\//i, "profile/")
    .replace(/^repairs\//i, "repairs/")
    .replace(/^videos\//i, "videos/")
    .replace(/^ecu_files\//i, "ecu_files/")
    .replace(/^gallery\//i, "gallery/")
    .replace(/^og_images\//i, "og_images/")
    .replace(/^fighter\//i, "fighter/");

  // Default profile ada di folder css kalau file default-profile.png belum ada di folder profile.
  if (folder === "profile" && clean === "default-profile.png") {
    return bucketPublicUrl("css/default-profile.png");
  }

  const known = ["css/", "profile/", "repairs/", "videos/", "ecu_files/", "gallery/", "og_images/", "fighter/", "remap_files/", "remap_results/"];
  if (known.some(prefix => clean.startsWith(prefix))) return bucketPublicUrl(clean);

  return bucketPublicUrl(`${folder}/${clean}`);
}


function ogMetaTags(request, { title = "IDS ECU REPAIR SOPPENG", description = "Forum komunitas teknisi ECU Indonesia.", image = "/media/og_images/og-banner.png", url = "/", type = "website" } = {}) {
  const absImage = absoluteUrl(request, image || "/media/og_images/og-banner.png");
  const absUrl = absoluteUrl(request, url || "/");
  return `
...
async function generateOgImageFromFirstImage(request, env, firstImageKey = "") {
  const key = r2KeyFromPublicPath(firstImageKey);
  if (!key || !key.startsWith("repairs/")) return "";
  const bucket = getR2Bucket(env);
  if (!bucket) return "";

  const ogKey = `og_images/og-${Date.now()}-${randomToken(4)}.jpg`;
  const sourceUrl = absoluteUrl(request, bucketPublicUrl(key));
  ...
  for (const q of qualities) {
    try {
      ...
      if (buf.byteLength <= 300 * 1024 || q === qualities[qualities.length - 1]) {
        await bucket.put(ogKey, buf, {
          httpMetadata: { contentType: "image/jpeg" },
          customMetadata: { type: "og_image", width: "1200", height: "630", max_size: "300kb" }
        });
        return ogKey;
      }
    } catch (e) {}
  ...
  // Fallback: kalau Cloudflare Image Resizing belum aktif, simpan copy gambar pertama ke folder og_images agar OG tetap muncul.
  try {
    const obj = await bucket.get(key);
    if (!obj) return "";
    await bucket.put(ogKey, obj.body, {
      httpMetadata: { contentType: obj.httpMetadata?.contentType || guessContentType(key) },
      customMetadata: { type: "og_image_fallback" }
    });
    return ogKey;
  } catch (e) {
    return "";
...
async function deleteR2KeyIfAllowed(env, value = "", defaultFolder = "") {
  const bucket = getR2Bucket(env);
  if (!bucket) return;

  const allowed = ["profile/", "repairs/", "videos/", "ecu_files/", "og_images/", "gallery/", "fighter/", "remap_files/", "remap_results/", "css/"];
  const rawKey = r2KeyFromPublicPath(value);
  if (!rawKey) return;

  const candidates = [];
  const addCandidate = (k) => {
    k = String(k || "").replace(/^\/+/, "");
    if (!k) return;
  ...
  // Data lama kadang hanya menyimpan nama file tanpa nama folder.
  // Contoh DB: image = abc.jpg, padahal object di R2 ada di repairs/abc.jpg.
  if (defaultFolder && !rawKey.includes("/")) {
    addCandidate(defaultFolder.replace(/\/+$/, "") + "/" + rawKey);
  }

  // Fallback untuk data lama yang pernah tersimpan dengan path /uploads/...
  const filename = rawKey.split("/").pop();
  if (filename) {
    if (defaultFolder) addCandidate(defaultFolder.replace(/\/+$/, "") + "/" + filename);
    if (/\.(jpg|jpeg|png|webp|gif)$/i.test(filename)) {
      addCandidate("repairs/" + filename);
      addCandidate("og_images/" + filename);
      addCandidate("profile/" + filename);
    }
    if (/\.(mp4|webm|ogg|mov)$/i.test(filename)) addCandidate("videos/" + filename);
    if (/\.(zip|rar|bin|ori|hex|fls|mpc|map|kess|ktag)$/i.test(filename)) addCandidate("ecu_files/" + filename);
  }

  for (const key of candidates) {
    if (!allowed.some(p => key.startsWith(p))) continue;
    try { await bucket.delete(key); } catch (e) {}
  }
}

async function deletePostFilesFromBucket(env, post) {
  const images = String(post.image || "").split(",").map(x => x.trim()).filter(Boolean);
  for (const img of images) await deleteR2KeyIfAllowed(env, img, "repairs");
  if (post.video) await deleteR2KeyIfAllowed(env, post.video, "videos");
  if (post.ecu_file) await deleteR2KeyIfAllowed(env, post.ecu_file, "ecu_files");
  if (post.og_image) await deleteR2KeyIfAllowed(env, post.og_image, "og_images");
}


function getR2Bucket(env) {
  return env.IDSREPAIR || env.BUCKET || env.R2 || env.R2_BUCKET || env.IDS_BUCKET || null;
}
...
function r2KeyFromPublicPath(url = "") {
  const value = String(url || "").trim();
  if (!value) return "";
  let clean = value;
  if (clean.startsWith(PUBLIC_FILE_ROUTE + "/")) {
    clean = clean.slice((PUBLIC_FILE_ROUTE + "/").length);
  } else if (/^https?:\/\//i.test(clean)) {
    try {
      clean = new URL(clean).pathname.replace(/^\/+/, "");
    } catch (e) {
      clean = "";
    }
  } else {
...
async function deleteOldProfilePhoto(env, oldPhotoUrl = "") {
  const bucket = getR2Bucket(env);
  if (!bucket) return;
  const oldKey = r2KeyFromPublicPath(oldPhotoUrl);
  if (!oldKey) return;
...
async function uploadProfilePhoto(env, userId, file, oldPhotoUrl = "") {
  ...
  const bucket = getR2Bucket(env);
  if (!bucket) throw new Error("Binding R2 belum ditemukan. Tambahkan R2 binding IDSREPAIR ke Worker.");
...
async function handleMediaFile(request, env) {
  try {
    const url = new URL(request.url);
    let key = decodeURIComponent(url.pathname.replace(/^\/media\/?/, "")).replace(/^\/+/, "");

    if (!key || key.includes("..") || key.startsWith(".")) {
      return new Response("File tidak valid", { status: 400 });
    }

    key = r2KeyFromPublicPath(key);
    ...
    if (key.startsWith("ecu_files/")) {
      const user = await requireLogin(request, env);
      if (!user) return redirectToLogin(request);
    } else if (!allowedPrefixes.some(prefix => key.startsWith(prefix))) {
      return new Response("Folder media tidak diizinkan: " + key, { status: 403 });
    }

    const bucket = getR2Bucket(env);
    if (!bucket) return new Response("Binding R2 belum ditemukan. Buat R2 binding dengan nama IDSREPAIR ke bucket idsrepair-images.", { status: 500 });

    const obj = await bucket.get(key);
    if (!obj) return new Response("File tidak ditemukan di R2: " + key, { status: 404 });

    const headers = new Headers();
    const contentType = obj.httpMetadata?.contentType || guessContentType(key);
    headers.set("Content-Type", contentType);
    headers.set("Cache-Control", key.startsWith("ecu_files/") ? "private, max-age=0, no-store" : "public, max-age=86400");
    headers.set("X-Content-Type-Options", "nosniff");
    headers.set("Accept-Ranges", "bytes");
    return new Response(obj.body, { status: 200, headers });
  } catch (e) {
    return new Response("Media Error: " + (e && e.message ? e.message : String(e)), { status: 500 });
  }
}
...
`).all();
  ...
  const postsHtml = (latest.results || []).map(p => {
    ...
    if (p.video) {
      const v = mediaCloudUrl(p.video.trim(), "videos");
...
<div class="post-card">
  ...
  <div class="post-content">
    ...
    ${fileBox}
    <div class="post-info">
      <img class="author-img" src="${esc(mediaCloudUrl(p.foto_profil || "default-profile.png", "profile"))}" loading="lazy" alt="Author Profile">
      <div>
...
<img src="/media/css/ecu-logo.png" alt="IDS ecu Repair Soppeng">
</div>
...
<img src="/media/gallery/banner-trusted-ai-ready.png" alt="IDS ECU Repair teruji terpercaya SEO Google dan AI Agent Ready" loading="lazy" class="trust-banner-img">
</div>
</section>
...
<img src="/media/gallery/ecu1.jpg" loading="lazy" alt="IDS Repair">
<img src="/media/gallery/ecu2.jpg" loading="lazy" alt="IDS Repair">
<img src="/media/gallery/ecu3.jpg" loading="lazy" alt="IDS Repair">
<video controls><source src="/media/gallery/video1.mp4" type="video/mp4"></video>
</div>
</div>
...
${ogMetaTags(request, { title: "IDS ECU REPAIR SOPPENG | Forum Repair ECU Indonesia", description: "Forum komunitas teknisi ECU Indonesia untuk repair ECU, file ECU, diagnosa, dan dokumentasi pekerjaan.", image: "/media/og_images/og-banner.png", url: "/", type: "website" })}
...
<link rel="icon" href="/media/css/ecu-logo.png" type="image/png">
...
`;
  ...
  // Pencarian aman untuk D1: kalau user ketik beberapa kata,
  // cukup satu kata cocok maka postingan akan muncul.
  const searchWords = q
    .toLowerCase()
    .split(/\s+/)
    .map(w => w.trim())
  ...
  if (searchWords.length) {
    ...
    for (const word of searchWords) {
      parts.push(`(
        lower(repair_posts.title) LIKE ?
        OR lower(repair_posts.content) LIKE ?
        OR lower(COALESCE(repair_posts.author,'')) LIKE ?
        OR lower(COALESCE(tamu.nama,'')) LIKE ?
        OR lower(COALESCE(repair_posts.ecu_file_original,'')) LIKE ?
      )`);
      const like = `%${word}%`;
...
${ecuFile}
<div class="info">
<img src="${esc(mediaCloudUrl(row.foto_profil || "default-profile.png", "profile"))}" loading="lazy" class="author-img" alt="Author">
<div>
...
`;
  ...
  return htmlResponse(pageWrap({ title: "Posting ECU Repair | IDS ECU REPAIR SOPPENG", head: ogMetaTags(request, { title: "Posting ECU Repair | IDS ECU REPAIR SOPPENG", description: "Daftar posting repair ECU, troubleshooting, file ECU, foto dan video pekerjaan teknisi IDS ECU REPAIR.", image: "/media/og_images/og-banner.png", url: "/posts/repair/", type: "website" }), styles, body, scripts }));
}
...
<img src="${esc(mediaCloudUrl(c.foto_profil || "default-profile.png", "profile"))}" class="comment-avatar">
<div>
...
<div class="post-info">
<img src="${esc(mediaCloudUrl(post.foto_profil || "default-profile.png", "profile"))}" class="author-img" alt="Foto profil">
<div>
...
document.addEventListener("DOMContentLoaded", function(){
  ...
  const viewOgImage = post.og_image ? mediaCloudUrl(post.og_image, "og_images") : (images[0] ? mediaCloudUrl(images[0], "repairs") : "/media/og_images/og-banner.png");
  return htmlResponse(pageWrap({
    title: `${esc(post.title || "")} | IDS ECU REPAIR`,
    head: ogMetaTags(request, { title: `${post.title || "Posting IDS ECU REPAIR"}`, description: String(post.content || "").slice(0, 160), image: viewOgImage, url: `/posts/view_repair/?id=${post.id}`, type: "article" }),
    styles,
    body,
    scripts
  }));
...
async function handleDownloadEcu(request, env) {
  ...
  const fileValue = String(post.ecu_file || "").trim();
  let key = r2KeyFromPublicPath(fileValue);
  if (!key.startsWith("ecu_files/")) key = `ecu_files/${key.replace(/^\/+/, "")}`;

  const bucket = getR2Bucket(env);
  if (!bucket) return new Response("Binding R2 belum ditemukan", { status: 500 });
  const obj = await bucket.get(key);
  if (!obj) return new Response("File tidak ditemukan di bucket", { status: 404 });

  const headers = new Headers();
  headers.set("Content-Type", obj.httpMetadata?.contentType || "application/octet-stream");
  headers.set("Content-Disposition", `attachment; filename="${String(post.ecu_file_original || key.split('/').pop()).replaceAll('"', '')}"`);
  headers.set("Cache-Control", "private, max-age=0, no-store");
  return new Response(obj.body, { headers });
}

async function handleDeleteRepair(request, env) {
  const user = await requireLogin(request, env);
  if (!user) return redirectToLogin(request);

  const url = new URL(request.url);
  const id = Number(url.searchParams.get("id") || 0);
  if (!id) return htmlAuthMessage("Hapus gagal", "ID posting tidak valid.", 400);

  const post = await env.DB.prepare("SELECT * FROM repair_posts WHERE id=? LIMIT 1").bind(id).first();
  if (!post) return htmlAuthMessage("Hapus gagal", "Postingan tidak ditemukan.", 404);

  const isOwner = Number(post.user_id || 0) === Number(user.id || 0);
  const isAdmin = String(user.role || "").toLowerCase() === "admin";
  if (!isOwner && !isAdmin) return htmlAuthMessage("Hapus gagal", "Anda tidak punya izin menghapus postingan ini.", 403);

  await deletePostFilesFromBucket(env, post);
  try { await env.DB.prepare("DELETE FROM repair_comments WHERE repair_id=?").bind(id).run(); } catch (e) {}
  await env.DB.prepare("DELETE FROM repair_posts WHERE id=?").bind(id).run();
...
async function handleCreateRepair(request, env) {
  ...
  if (request.method === "POST") {
    try {
      ...
      if (!title || !content) {
        return htmlAuthMessage("Posting gagal", "Judul dan isi posting wajib diisi.", 400);
      }

      const bucket = getR2Bucket(env);
      if (!bucket) {
        return htmlAuthMessage("Posting gagal", "Binding R2 belum ditemukan. Buat binding IDSREPAIR ke bucket idsrepair-images.", 500);
      }

      const imageFiles = form.getAll("image[]").filter(f => f && typeof f === "object" && f.size > 0);
      const savedImages = [];

      if (imageFiles.length > 10) {
        return htmlAuthMessage("Posting gagal", "Maksimal upload 10 gambar dalam satu posting.", 400);
      }

      for (const file of imageFiles) {
        if (!isAllowedImageType(file)) {
          return htmlAuthMessage("Posting gagal", "Format gambar harus JPG, PNG, WEBP, atau GIF.", 400);
        }
        if (file.size > 10 * 1024 * 1024) {
          return htmlAuthMessage("Posting gagal", "Ukuran tiap gambar maksimal 10MB.", 400);
        }
        const ext = safeFileExt(file.name, "jpg");
        const key = `repairs/post-${user.id}-${Date.now()}-${randomToken(4)}.${ext}`;
        await bucket.put(key, file.stream(), {
          httpMetadata: { contentType: file.type || guessContentType(key) },
          customMetadata: { user_id: String(user.id), type: "repair_image" }
        });
      ...
      if (video && typeof video === "object" && video.size > 0) {
        const vName = String(video.name || "").toLowerCase();
        const vType = String(video.type || "").toLowerCase();
        if (!(vType.startsWith("video/") || /\.(mp4|webm|ogg)$/i.test(vName))) {
          return htmlAuthMessage("Posting gagal", "Format video harus MP4, WEBM, atau OGG.", 400);
        }
        if (video.size > 80 * 1024 * 1024) {
          return htmlAuthMessage("Posting gagal", "Ukuran video maksimal 80MB.", 400);
        }
        const ext = safeFileExt(video.name, "mp4");
        const key = `videos/post-${user.id}-${Date.now()}-${randomToken(4)}.${ext}`;
        await bucket.put(key, video.stream(), {
          httpMetadata: { contentType: video.type || guessContentType(key) },
          customMetadata: { user_id: String(user.id), type: "repair_video" }
        });
        savedVideo = key;
      }
      ...
      if (ecuFile && typeof ecuFile === "object" && ecuFile.size > 0) {
        const eName = String(ecuFile.name || "");
        if (!/\.(zip|rar)$/i.test(eName)) {
          return htmlAuthMessage("Posting gagal", "File ECU harus format ZIP atau RAR.", 400);
        }
        if (ecuFile.size > 80 * 1024 * 1024) {
          return htmlAuthMessage("Posting gagal", "Ukuran file ECU maksimal 80MB.", 400);
        }
        const ext = safeFileExt(ecuFile.name, "zip");
        ecuKey = `ecu_files/ecu-${user.id}-${Date.now()}-${randomToken(4)}.${ext}`;
        ecuOriginal = eName.replace(/[\\/]/g, "_");
        ecuSize = Number(ecuFile.size || 0);
        await bucket.put(ecuKey, ecuFile.stream(), {
          httpMetadata: { contentType: ecuFile.type || guessContentType(ecuKey) },
          customMetadata: { user_id: String(user.id), original_name: ecuOriginal, type: "ecu_file" }
        });
      }

      const ogImage = savedImages[0] ? await generateOgImageFromFirstImage(request, env, savedImages[0]) : "";
      const columnsInfo = await env.DB.prepare("PRAGMA table_info(repair_posts)").all();
      const existing = new Set((columnsInfo.results || []).map(c => c.name));
      const data = {
        user_id: user.id,
        author: user.nama || user.email || "Member",
        title,
        content,
        image: savedImages.join(","),
        video: savedVideo,
        youtube_url: youtubeUrl,
        og_image: ogImage,
        ecu_file: ecuKey,
        ecu_file_original: ecuOriginal,
        ecu_file_size: ecuSize,
        pinned: 0,
        created_at: new Date().toISOString()
      };

      const cols = Object.keys(data).filter(k => existing.has(k));
      const placeholders = cols.map(() => "?").join(",");
      const values = cols.map(k => data[k]);
      await env.DB.prepare(`INSERT INTO repair_posts (${cols.join(",")}) VALUES (${placeholders})`).bind(...values).run();

      return Response.redirect(new URL("/dashboard/my_posts/", request.url), 302);
    } catch (e) {
      return htmlAuthMessage("Posting gagal", e.message || "Terjadi kesalahan saat membuat posting.", 500);
    }
  }
...
async function handleDashboard(request, env) {
  const user = await requireLogin(request, env);
  if (!user) return redirectToLogin(request);
  await attachRemapOrdersToUser(env, user);
...
WHERE repair_posts.user_id=?`).bind(userId).first();
  ...
  try {
    totalRemapOrders = await env.DB.prepare("SELECT COUNT(*) AS total FROM remap_orders WHERE user_id=? OR lower(email)=?").bind(userId, userEmail).first();
  } catch(e) {}
  try {
    myRemapOrders = await env.DB.prepare("SELECT * FROM remap_orders WHERE user_id=? OR lower(email)=? ORDER BY id DESC LIMIT 6").bind(userId, userEmail).all();
  } catch(e) {}
  try {
    myCompletedRemapFiles = await env.DB.prepare("SELECT * FROM remap_orders WHERE (user_id=? OR lower(email)=?) AND result_file IS NOT NULL AND result_file!='' ORDER BY id DESC LIMIT 6").bind(userId, userEmail).all();
  } catch(e) {}
  try {
    remapNotifications = await env.DB.prepare("SELECT * FROM remap_notifications WHERE user_id=? ORDER BY id DESC LIMIT 5").bind(userId).all();
  } catch(e) {}
...
<div class="comment-item">
<div class="comment-top">
<img src="${esc(mediaCloudUrl(c.foto_profil || "default-profile.png", "profile"))}">
<div>
...
<img src="${esc(mediaCloudUrl(user?.foto_profil || "default-profile.png", "profile"))}" class="profile-img">
...
<img src="${esc(mediaCloudUrl(user?.foto_profil || "default-profile.png", "profile"))}" class="dropdown-img">
...
<img src="${esc(mediaCloudUrl(user?.foto_profil || "default-profile.png", "profile"))}">
<div>
<h1>Panel Teknisi</h1>
...
<a class="delete" href="/posts/delete_repair.php?id=${p.id}" onclick="return confirm('Yakin hapus postingan ini? Semua foto, video, file ECU, dan OG image di bucket juga akan dihapus.')">Hapus</a>
</div>
</div>
</div>`;
...
<img src="${esc(mediaCloudUrl(shownUser?.foto_profil || "default-profile.png", "profile"))}">
...
<img src="/media/fighter/awaluddin-soppeng.png" alt="Awaludin Supernew">
...
const REMAP_PRICE_DEFAULT = 1500000;

function remapPrice(env) {
  return Number(env.REMAP_PRICE || REMAP_PRICE_DEFAULT) || REMAP_PRICE_DEFAULT;
}
...
async function ensureRemapTables(env) {
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS remap_orders (
    ...
    permintaan TEXT,
    ecu_file TEXT,
    ecu_file_original TEXT,
    result_file TEXT,
    result_file_original TEXT,
  ...
  for (const [name, type] of columns) {
    try { await env.DB.prepare(`ALTER TABLE remap_orders ADD COLUMN ${name} ${type}`).run(); } catch (e) {}
  }

  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS remap_notifications (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER,
    order_id INTEGER,
    title TEXT,
...
async function addRemapNotification(env, userId, orderId, title, message) {
  userId = Number(userId || 0);
  orderId = Number(orderId || 0);
  if (!userId || !orderId) return;
  await ensureRemapTables(env);
  try {
    await env.DB.prepare(`INSERT INTO remap_notifications (user_id,order_id,title,message,is_read) VALUES (?,?,?,?,0)`)
      .bind(userId, orderId, title, message).run();
  } catch (e) {}
}


async function attachRemapOrdersToUser(env, user) {
...
function isAllowedRemapOriginal(file) {
  const name = String(file?.name || "").toLowerCase();
  return /\.(bin|ori|hex|zip|rar)$/i.test(name);
}

function isAllowedRemapResult(file) {
  const name = String(file?.name || "").toLowerCase();
  return /\.(bin|ori|hex|zip|rar|pdf|txt)$/i.test(name);
}

async function uploadRemapFile(env, file, folder, prefix) {
  if (!file || !file.size) throw new Error("File belum dipilih.");
  const bucket = getR2Bucket(env);
  if (!bucket) throw new Error("Binding R2 belum ditemukan. Buat binding IDSREPAIR ke bucket idsrepair-images.");
  const ext = safeFileExt(file.name, "bin");
  const key = `${folder}/${prefix}-${Date.now()}-${randomToken(4)}.${ext}`;
  await bucket.put(key, file.stream(), {
    httpMetadata: { contentType: file.type || guessContentType(key) },
    customMetadata: { original_name: String(file.name || ""), type: folder }
  });
  return key;
}

function remapEmailTemplate(title, bodyHtml, buttonText = "", buttonUrl = "") {
  return `<!DOCTYPE html><html lang="id"><body style="margin:0;background:#0d0d0d;font-family:Arial,sans-serif;color:#fff;padding:24px;">
  <div style="max-width:560px;margin:auto;background:#111;border:1px solid #c00000;border-radius:16px;padding:24px;box-shadow:0 0 22px rgba(255,0,0,.16);">
  <h2 style="color:#ff2b2b;text-align:center;margin:0 0 18px;">${esc(title)}</h2>
  <div style="color:#ddd;font-size:14px;line-height:1.8;">${bodyHtml}</div>
  ${buttonUrl ? `<div style="text-align:center;margin:24px 0;"><a href="${esc(buttonUrl)}" style="display:inline-block;background:#c00000;color:#fff;text-decoration:none;padding:13px 18px;border-radius:10px;font-weight:bold;">${esc(buttonText || "BUKA")}</a></div><p style="font-size:12px;color:#999;line-height:1.6;">Jika tombol tidak terbuka, salin link ini:<br>${esc(buttonUrl)}</p>` : ""}
  <p style="margin-top:20px;color:#999;font-size:12px;line-height:1.6;">IDS ECU REPAIR SOPPENG</p>
  </div></body></html>`;
}

async function sendRemapOrderEmail(request, env, order, paymentUrl = "") {
  const html = remapEmailTemplate(
    "Order Remap Dibuat",
    `Hai <b>${esc(order.nama)}</b>,<br><br>Order remap ECU kamu sudah berhasil dibuat dan sedang menunggu pembayaran.<br><br><b>Detail order:</b><br>Kendaraan: ${esc(order.kendaraan)}<br>Tipe ECU: ${esc(order.ecu_type)}<br>Permintaan: ${nl2brEsc(order.permintaan || "-")}<br>Nomor Order: #${esc(order.id)}<br>Total: ${rupiah(order.price)}<br><br>Silakan selesaikan pembayaran melalui halaman Duitku.`,
    "BAYAR SEKARANG",
    paymentUrl
  );
  await sendResendEmail(env, { to: order.email, subject: "Order Remap Dibuat - Menunggu Pembayaran", html });
}

async function sendRemapPaidEmail(env, order) {
  const html = remapEmailTemplate(
    "Pembayaran Remap Berhasil",
    `Hai <b>${esc(order.nama)}</b>,<br><br>Pembayaran order remap ECU kamu sudah berhasil kami terima.<br><br><b>Detail order:</b><br>Kendaraan: ${esc(order.kendaraan)}<br>Tipe ECU: ${esc(order.ecu_type)}<br>Nomor Order: #${esc(order.id)}<br><br>File ECU kamu sedang diproses. Estimasi pengerjaan maksimal <b>1 x 24 jam</b>.`
  );
  await sendResendEmail(env, { to: order.email, subject: "Pembayaran Remap Berhasil - IDS ECU REPAIR", html });
}

async function sendRemapResultEmail(request, env, order) {
  const resultUrl = absoluteUrl(request, `/remap/download.php?id=${encodeURIComponent(order.id)}&type=result`);
  const html = remapEmailTemplate(
    "File Hasil Remap Selesai",
    `Hai <b>${esc(order.nama)}</b>,<br><br>File hasil remap ECU kamu sudah selesai.<br><br><b>Detail order:</b><br>Kendaraan: ${esc(order.kendaraan)}<br>Tipe ECU: ${esc(order.ecu_type)}<br>Nomor Order: #${esc(order.id)}<br><br>Silakan download file hasil melalui tombol di bawah ini.`,
    "DOWNLOAD HASIL REMAP",
    resultUrl
  );
  await sendResendEmail(env, { to: order.email, subject: "File Hasil Remap Selesai - IDS ECU REPAIR", html });
}

async function duitkuCreateInvoice(request, env, order) {
  const merchant = env.DUITKU_MERCHANT || env.DUITKU_MERCHANT_CODE || "";
  const apiKey = env.DUITKU_API_KEY || env.DUITKU_APIKEY || "";
  const endpoint = env.DUITKU_ENDPOINT || "https://api-prod.duitku.com/api/merchant/createInvoice";
  if (!merchant || !apiKey) {
    return { skipped: true, paymentUrl: "" };
  }

  const timestamp = String(Date.now());
  const signature = await sha256Text(merchant + timestamp + apiKey);
  const origin = new URL(request.url).origin;
  const params = {
    paymentAmount: Number(order.price || remapPrice(env)),
    merchantOrderId: order.merchant_order_id,
    productDetails: "Remap ECU IDSREPAIR",
    additionalParam: "",
    merchantUserInfo: order.email,
    customerVaName: String(order.nama || "IDSREPAIR").slice(0, 20),
    email: order.email,
    phoneNumber: order.whatsapp,
    itemDetails: [{ name: "Remap ECU IDSREPAIR", price: Number(order.price || remapPrice(env)), quantity: 1 }],
    customerDetail: { firstName: order.nama, lastName: "", email: order.email, phoneNumber: order.whatsapp },
    callbackUrl: `${origin}/remap/duitku-callback`,
    returnUrl: `${origin}/remap/?payment_return=1`,
    expiryPeriod: 60
  };
  ...
  if (!res.ok || !data.paymentUrl) {
    throw new Error("Gagal membuat pembayaran Duitku: " + JSON.stringify(data));
  }
  return data;
}

async function handleDuitkuCallback(request, env) {
  await ensureRemapTables(env);
  ...
  const merchantCode = String(data.merchantCode || "");
  ...
  const expected = await md5Text(merchantCode + amount + merchantOrderId + apiKey);
  if (apiKey && signature && signature !== expected) {
    return new Response("BAD SIGNATURE", { status: 400 });
  }

  const order = await env.DB.prepare(`SELECT * FROM remap_orders WHERE merchant_order_id=? OR duitku_reference=? LIMIT 1`).bind(merchantOrderId, reference).first();
  if (!order) return new Response("ORDER NOT FOUND", { status: 404 });
  const alreadyPaid = String(order.payment_status || "") === "paid";

  if (resultCode === "00") {
    await env.DB.prepare(`UPDATE remap_orders SET payment_status='paid', status='diproses', duitku_reference=COALESCE(NULLIF(duitku_reference,''),?), callback_log=? WHERE id=?`)
      .bind(reference, JSON.stringify(data), order.id).run();
    if (!alreadyPaid) {
      const updated = await env.DB.prepare(`SELECT * FROM remap_orders WHERE id=?`).bind(order.id).first();
      await addRemapNotification(env, updated.user_id, updated.id, "Pembayaran Remap Berhasil", `Pembayaran order remap ${updated.kendaraan} - ${updated.ecu_type} berhasil. File ECU masuk proses.`);
      try { await sendRemapPaidEmail(env, updated); } catch (e) {}
    }
  } else {
    await env.DB.prepare(`UPDATE remap_orders SET payment_status='failed', status='menunggu_pembayaran', callback_log=? WHERE id=?`).bind(JSON.stringify(data), order.id).run();
  }
...
function md5Text(text = "") {
  ...
  function md5cycle(x, k) {
    ...
    x[0] = (a + oa) | 0; x[1] = (b + ob) | 0; x[2] = (c + oc) | 0; x[3] = (d + od) | 0;
  }
  function md5blk(s) {
    const md5blks = [];
...
async function handleRemapDownload(request, env) {
  await ensureRemapTables(env);
  const user = await requireLogin(request, env);
  if (!user) return redirectToLogin(request);
  const url = new URL(request.url);
  const id = Number(url.searchParams.get("id") || 0);
  const type = String(url.searchParams.get("type") || "original");
  const order = await env.DB.prepare(`SELECT * FROM remap_orders WHERE id=? LIMIT 1`).bind(id).first();
  if (!order) return new Response("Order tidak ditemukan", { status: 404 });
  const isOwner = Number(order.user_id || 0) === Number(user.id || 0) || String(order.email || "").toLowerCase() === String(user.email || "").toLowerCase();
  if (!isOwner && user.role !== "admin") return new Response("Akses ditolak", { status: 403 });

  const key = type === "result" ? r2KeyFromPublicPath(order.result_file || "") : r2KeyFromPublicPath(order.ecu_file || "");
  const originalName = type === "result" ? (order.result_file_original || "hasil-remap.bin") : (order.ecu_file_original || "ecu-original.bin");
  if (!key) return new Response("File belum tersedia", { status: 404 });
  const bucket = getR2Bucket(env);
  if (!bucket) return new Response("Binding R2 belum ditemukan", { status: 500 });
  const obj = await bucket.get(key);
  if (!obj) return new Response("File tidak ditemukan di R2", { status: 404 });
  const headers = new Headers();
  headers.set("Content-Type", obj.httpMetadata?.contentType || guessContentType(key));
  headers.set("Content-Disposition", `attachment; filename="${String(originalName).replace(/[\r\n"]/g, "")}"`);
  headers.set("Cache-Control", "private, no-store");
  return new Response(obj.body, { headers });
}

async function handleRemap(request, env) {
  await ensureRemapTables(env);
  const currentUser = await getCurrentUser(request, env);
  const url = new URL(request.url);
  ...
  if (request.method === "POST") {
    const form = await request.formData();
    const nama = cleanText(form.get("nama"));
    const whatsapp = cleanText(form.get("whatsapp"));
    const email = cleanEmail(form.get("email"));
    const kendaraan = cleanText(form.get("kendaraan"));
    const ecu_type = cleanText(form.get("ecu_type"));
    const permintaan = cleanText(form.get("permintaan"));
    const file = form.get("ecu_file");

    const fail = (msg) => Response.redirect(`${url.origin}/remap/?remap_error=${encodeURIComponent(msg)}`, 302);
    if (!nama || !whatsapp || !email || !kendaraan || !ecu_type) return fail("Data wajib belum lengkap.");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return fail("Format email tidak valid.");
    if (!file || !file.size) return fail("File ECU original wajib diupload.");
    if (file.size > 150 * 1024 * 1024) return fail("Ukuran file ECU maksimal 150MB.");
    if (!isAllowedRemapOriginal(file)) return fail("Format file ECU harus BIN, ORI, HEX, ZIP, atau RAR.");

    let key = "";
    try {
      key = await uploadRemapFile(env, file, "remap_files", "original");
      const price = remapPrice(env);
      const insert = await env.DB.prepare(`INSERT INTO remap_orders
        (user_id,nama,whatsapp,email,kendaraan,ecu_type,permintaan,ecu_file,ecu_file_original,price,status,payment_status)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`)
        .bind(currentUser?.id || 0, nama, whatsapp, email, kendaraan, ecu_type, permintaan, key, file.name || "ecu-original.bin", price, "menunggu_pembayaran", "pending").run();
      const orderId = insert.meta.last_row_id;
      const merchantOrderId = `REMAP${orderId}${Date.now()}`;
      await env.DB.prepare(`UPDATE remap_orders SET merchant_order_id=? WHERE id=?`).bind(merchantOrderId, orderId).run();
      let order = await env.DB.prepare(`SELECT * FROM remap_orders WHERE id=?`).bind(orderId).first();

      let paymentUrl = "";
      try {
        const result = await duitkuCreateInvoice(request, env, order);
        paymentUrl = result.paymentUrl || "";
        if (result.reference) {
          await env.DB.prepare(`UPDATE remap_orders SET duitku_reference=?, payment_url=? WHERE id=?`).bind(result.reference, paymentUrl, orderId).run();
        } else if (paymentUrl) {
          await env.DB.prepare(`UPDATE remap_orders SET payment_url=? WHERE id=?`).bind(paymentUrl, orderId).run();
        }
      } catch (e) {
        await env.DB.prepare(`UPDATE remap_orders SET callback_log=? WHERE id=?`).bind(String(e.message || e), orderId).run();
      }

      order = await env.DB.prepare(`SELECT * FROM remap_orders WHERE id=?`).bind(orderId).first();
      await addRemapNotification(env, order.user_id, order.id, "Order Remap Dibuat", `Order remap ${kendaraan} - ${ecu_type} berhasil dibuat.`);
      try { await sendRemapOrderEmail(request, env, order, paymentUrl); } catch (e) {}

      if (paymentUrl) return Response.redirect(paymentUrl, 302);
      return Response.redirect(`${url.origin}/remap/?remap_success=${encodeURIComponent("Order berhasil dibuat. API Duitku belum aktif atau belum lengkap, order tersimpan di admin.")}`, 302);
    } catch (e) {
      if (key) { try { await getR2Bucket(env)?.delete(key); } catch (x) {} }
      return fail(e.message || "Gagal membuat order remap.");
    }
  }

  const success = url.searchParams.get("remap_success") || (url.searchParams.has("payment_return") ? "Jika pembayaran berhasil, order kamu akan otomatis masuk proses. Cek email notifikasi atau dashboard." : "");
  const error = url.searchParams.get("remap_error") || "";

  const styles = `*{margin:0;padding:0;box-sizing:border-box;}body{background:linear-gradient(rgba(0,0,0,.92),rgba(0,0,0,.94)),url('/media/css/bg-ecu.png');font-family:Arial,sans-serif;color:#fff;}.header{position:sticky;top:0;z-index:99;background:#111;padding:10px 14px;border-bottom:2px solid #c00000;display:flex;justify-content:space-between;align-items:center;border:none!important;box-shadow:0 0 14px rgba(255,0,0,.45);}.header-title{font-weight:bold;margin-left:14px;color:#ff2b2b;font-size:17px;letter-spacing:1px;}.back{background:#1d1d1d;border:1px solid #333;padding:7px 11px;border-radius:10px;text-decoration:none;color:#fff;font-size:13px;margin-right:10px;}.container{width:100%;max-width:880px;margin:auto;padding:16px;}.hero{text-align:center;padding:24px 0 15px;}.banner{width:100%;height:220px;object-fit:cover;border-radius:18px;margin-bottom:20px;border:1px solid #222;box-shadow:0 0 20px rgba(255,0,0,.15);opacity:0;transform:scale(1.03);animation:bannerLoad .9s ease forwards;}.hero h1{font-size:26px;color:#ff2b2b;line-height:1.35;margin-bottom:12px;}.hero p{font-size:14px;line-height:1.8;color:#ccc;max-width:760px;margin:auto;}.card{background:#161616;border:1px solid #242424;border-radius:16px;padding:18px;margin-bottom:16px;box-shadow:0 0 18px rgba(255,0,0,.04);}.card h2{font-size:19px;color:#ff3c3c;margin-bottom:12px;}.card p,.card li{font-size:14px;line-height:1.8;color:#ccc;}.card ul{padding-left:20px;}.service-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-top:14px;}.service-box{background:#101010;border:1px solid #2a2a2a;border-radius:12px;padding:13px;font-size:13px;line-height:1.6;color:#ddd;}.price-box{margin-top:15px;background:#101010;border:1px solid #333;border-radius:14px;padding:15px;text-align:center;}.price-box span{display:block;font-size:13px;color:#aaa;margin-bottom:7px;}.price-box b{font-size:24px;color:#00ff7b;}.continue-btn{width:100%;background:#c00000;color:#fff;border:none;border-radius:12px;padding:15px;font-size:15px;font-weight:bold;cursor:pointer;margin-top:14px;box-shadow:0 0 18px rgba(255,0,0,.2);}.continue-btn:hover{background:#ff2b2b;}.alert{padding:11px 14px;border-radius:12px;margin:0 auto 15px;font-size:13px;line-height:1.55;text-align:center;font-weight:bold;width:100%;max-width:520px;}.alert-success{background:#005c2a;color:#fff;}.alert-error{background:#9b0000;color:#fff;white-space:pre-wrap;text-align:left;}#orderForm{display:none;}.form-group{margin-bottom:14px;}.form-group label{display:block;font-size:13px;color:#bbb;margin-bottom:7px;}.form-group input,.form-group textarea{width:100%;background:#1b1b1b;border:1px solid #333;border-radius:10px;padding:13px;color:#fff;font-size:14px;outline:none;}.form-group textarea{min-height:115px;resize:vertical;}.form-group input:focus,.form-group textarea:focus{border-color:#c00000;box-shadow:0 0 0 2px rgba(255,0,0,.12);}.note{font-size:12px;color:#888;line-height:1.7;margin-top:8px;}.submit-btn{width:100%;background:#c00000;border:none;border-radius:12px;padding:15px;color:#fff;font-weight:bold;font-size:15px;cursor:pointer;}.submit-btn:hover{background:#ff2b2b;}.footer-note{font-size:12px;color:#777;line-height:1.7;text-align:center;padding:10px 0 25px;}@keyframes bannerLoad{0%{opacity:0;transform:scale(1.05);filter:blur(8px);}100%{opacity:1;transform:scale(1);filter:blur(0);}}@media(max-width:600px){.banner{height:150px;}.hero h1{font-size:21px;}.card{padding:15px;}.price-box b{font-size:21px;}}`;
  const body = `<div class="header"><div class="header-title">⚡ Remap ECU</div><a href="/" class="back">Home</a></div><div class="container">
${success ? `<div class="alert alert-success">${esc(success)}</div>` : ""}
${error ? `<div class="alert alert-error">${esc(error)}</div>` : ""}
<section class="hero"><img src="/media/css/og-banner-remap.png" class="banner" alt="Remap ECU IDS Repair"><h1>Remap ECU & Penambahan Tenaga Mesin</h1><p>Layanan tuning file ECU untuk meningkatkan performa mesin, respons pedal, torsi, dan karakter tenaga kendaraan. File original kamu akan diproses oleh teknisi, lalu hasil setting dikirim kembali ke email yang kamu isi.</p></section>
<div class="card"><h2>Apa Itu Remap ECU?</h2><p>Remap ECU adalah proses pengaturan ulang data pada file ECU kendaraan. Tujuannya untuk mengoptimalkan tenaga, torsi, respons akselerasi, dan karakter mesin sesuai kebutuhan kendaraan.</p><br><p>Layanan ini cocok untuk kendaraan diesel/common rail, mobil harian, unit operasional, kendaraan niaga, dan kebutuhan tuning ringan sampai performa.</p><div class="price-box"><span>Biaya layanan per 1 file ECU</span><b>${rupiah(remapPrice(env))}</b></div><button type="button" class="continue-btn" onclick="showOrderForm()">LANJUTKAN ORDER REMAP</button></div>
<div class="card"><h2>Layanan Yang Bisa Diminta</h2><div class="service-grid"><div class="service-box">⚡ <b>Stage 1</b><br>Optimasi tenaga dan torsi untuk penggunaan harian.</div><div class="service-box">🚗 <b>Throttle Response</b><br>Respons pedal lebih cepat dan tarikan lebih ringan.</div><div class="service-box">🛠 <b>EGR / DPF Request</b><br>Penanganan sesuai kebutuhan repair dan troubleshooting.</div><div class="service-box">📁 <b>File Checking</b><br>Pemeriksaan file original sebelum proses tuning.</div></div></div>
<div class="card"><h2>Alur Order</h2><ul><li>Isi data kendaraan dan kontak penerima.</li><li>Upload file ECU original.</li><li>Sistem membuka halaman pilihan pembayaran.</li><li>Setelah pembayaran berhasil, status order otomatis masuk proses.</li><li>Admin memproses file dan mengirim hasil ke email kamu.</li></ul></div>
<div id="orderForm"><div class="card"><h2>Form Order Remap ECU</h2><div class="service-box">💳 <b>Pembayaran Otomatis</b><br>Mendukung QRIS, E-Wallet, transfer bank, dan minimarket dengan verifikasi otomatis.</div><form method="POST" enctype="multipart/form-data"><div class="form-group"><label>Nama Lengkap</label><input type="text" name="nama" value="${esc(currentUser?.nama || "")}" required></div><div class="form-group"><label>Nomor WhatsApp</label><input type="text" name="whatsapp" placeholder="Contoh: 082346783838" required></div><div class="form-group"><label>Email Penerima File Hasil Setting</label><input type="email" name="email" value="${esc(currentUser?.email || "")}" placeholder="contoh@email.com" required></div><div class="form-group"><label>Jenis Kendaraan</label><input type="text" name="kendaraan" placeholder="Contoh: Fortuner 2GD / Hilux / Pajero" required></div><div class="form-group"><label>Tipe ECU</label><input type="text" name="ecu_type" placeholder="Contoh: Denso / Bosch / Delphi / Continental" required></div><div class="form-group"><label>Permintaan Setting</label><textarea name="permintaan" placeholder="Contoh: Stage 1, tenaga bawah tambah, EGR OFF, DPF OFF, torque naik, dll"></textarea></div><div class="form-group"><label>Upload File ECU Original</label><input type="file" name="ecu_file" accept=".bin,.ori,.hex,.zip,.rar" required><div class="note">Format: BIN, ORI, HEX, ZIP, RAR. Maksimal 150MB.</div></div><button type="submit" class="submit-btn">KIRIM ORDER & BAYAR SEKARANG</button></form></div></div>
<div class="footer-note">IDS ECU REPAIR SOPPENG — Remap ECU, tuning file, dan penambahan tenaga mesin.</div></div>`;
...
async function handleAdminRemap(request, env) {
  ...
  if (String(user.role || "") !== "admin") {
    return htmlAuthMessage("Akses Ditolak", "Halaman admin remap hanya bisa dibuka oleh admin.", 403);
  }
  ...
  function adminRedirect(params) {
    const u = new URL("/remap/admin/", url.origin);
    const filterNow = url.searchParams.get("filter") || "menunggu_pembayaran";
    u.searchParams.set("filter", filterNow);
  ...
  if (request.method === "POST") {
    ...
    const order = orderId ? await env.DB.prepare(`SELECT * FROM remap_orders WHERE id=? LIMIT 1`).bind(orderId).first() : null;
    if (!order) return adminRedirect({ error: "Order tidak ditemukan." });

    if (action === "update_status") {
      const status = String(form.get("status") || "");
      const allowed = ["menunggu_pembayaran", "diproses", "selesai", "ditolak"];
      if (!allowed.includes(status)) return adminRedirect({ error: "Status tidak valid." });
      await env.DB.prepare(`UPDATE remap_orders SET status=? WHERE id=?`).bind(status, orderId).run();
      return adminRedirect({ success: "Status order berhasil diperbarui." });
    }

    if (action === "mark_paid") {
      await env.DB.prepare(`UPDATE remap_orders SET payment_status='paid', status='diproses' WHERE id=?`).bind(orderId).run();
      const updated = await env.DB.prepare(`SELECT * FROM remap_orders WHERE id=?`).bind(orderId).first();
      await addRemapNotification(env, updated.user_id, updated.id, "Pembayaran Remap Berhasil", `Pembayaran order remap ${updated.kendaraan || ""} - ${updated.ecu_type || ""} ditandai berhasil oleh admin.`);
      try { await sendRemapPaidEmail(env, updated); } catch (e) {}
      return adminRedirect({ success: "Order ditandai paid dan masuk proses." });
    }

    if (action === "upload_result") {
      const file = form.get("result_file");
      if (!file || !file.size) return adminRedirect({ error: "File hasil remap wajib diupload." });
      if (file.size > 150 * 1024 * 1024) return adminRedirect({ error: "Ukuran file hasil maksimal 150MB." });
      if (!isAllowedRemapResult(file)) return adminRedirect({ error: "Format hasil harus BIN, ORI, HEX, ZIP, RAR, PDF, atau TXT." });

      if (order.result_file) {
        try { await getR2Bucket(env)?.delete(r2KeyFromPublicPath(order.result_file)); } catch (e) {}
      }

      const resultKey = await uploadRemapFile(env, file, "remap_results", "result");
      await env.DB.prepare(`
UPDATE remap_orders
SET result_file=?, result_file_original=?, status='selesai', payment_status=CASE WHEN payment_status='paid' THEN payment_status ELSE 'paid' END, result_sent_at=?
WHERE id=?
`).bind(resultKey, file.name || "hasil-remap.bin", new Date().toISOString(), orderId).run();
      const updated = await env.DB.prepare(`SELECT * FROM remap_orders WHERE id=?`).bind(orderId).first();
      await addRemapNotification(env, updated.user_id, updated.id, "File Remap ECU Selesai", `File hasil remap untuk ${updated.kendaraan || "kendaraan"} sudah selesai. Silakan download hasilnya.`);
      let sent = true;
      try { await sendRemapResultEmail(request, env, updated); } catch (e) { sent = false; }
      return adminRedirect({ success: sent ? "File hasil remap berhasil diupload, status selesai, dan email hasil terkirim." : "File hasil remap berhasil diupload, tapi email gagal terkirim. Cek RESEND_API_KEY." });
    }

    if (action === "delete") {
      for (const key of [order.ecu_file, order.result_file, order.payment_proof]) {
        if (key) { try { await getR2Bucket(env)?.delete(r2KeyFromPublicPath(key)); } catch (e) {} }
      }
      try { await env.DB.prepare(`DELETE FROM remap_notifications WHERE order_id=?`).bind(orderId).run(); } catch (e) {}
      await env.DB.prepare(`DELETE FROM remap_orders WHERE id=?`).bind(orderId).run();
      return adminRedirect({ success: "Order remap berhasil dihapus dari database dan file bucket." });
    }
  }

  const filter = ["menunggu_pembayaran", "diproses", "selesai", "ditolak", "semua"].includes(url.searchParams.get("filter")) ? url.searchParams.get("filter") : "menunggu_pembayaran";
  const countRows = await env.DB.prepare(`SELECT status, COUNT(*) AS total FROM remap_orders GROUP BY status`).all();
  const counts = { menunggu_pembayaran: 0, diproses: 0, selesai: 0, ditolak: 0, semua: 0 };
  for (const r of (countRows.results || [])) {
    if (counts[r.status] !== undefined) counts[r.status] = Number(r.total || 0);
    counts.semua += Number(r.total || 0);
  }

  let orders;
  if (filter === "semua") orders = await env.DB.prepare(`SELECT * FROM remap_orders ORDER BY id DESC`).all();
  else orders = await env.DB.prepare(`SELECT * FROM remap_orders WHERE status=? ORDER BY id DESC`).bind(filter).all();
  ...
  const tab = (key, title) => `<a class="tab ${filter === key ? "active" : ""}" href="/remap/admin/?filter=${key}"><div class="tab-title">${title}</div><div class="tab-count">${counts[key] || 0}</div></a>`;

  const styles = `*{margin:0;padding:0;box-sizing:border-box;}body{background:#0d0d0d;color:#fff;font-family:Arial,sans-serif;}.header{position:sticky;top:0;z-index:99;background:#111;border-bottom:2px solid #c00000;padding:14px 15px;display:flex;align-items:center;justify-content:space-between;gap:10px;box-shadow:0 0 20px rgba(255,0,0,.12);}.header-title{font-size:16px;font-weight:bold;color:#ff2b2b;line-height:1.4;}.header-user{font-size:11px;color:#888;margin-top:3px;}.nav-btn{background:#1b1b1b;border:1px solid #333;border-radius:9px;padding:8px 12px;color:#fff;text-decoration:none;font-size:12px;white-space:nowrap;}.nav-btn:hover{background:#ff2b2b;}.container{width:100%;max-width:1050px;margin:auto;padding:14px;}.alert{padding:12px;border-radius:10px;margin-bottom:14px;font-size:13px;line-height:1.6;font-weight:bold;}.success{background:#006b2d;}.error{background:#9b0000;}.admin-version{font-size:11px;color:#777;margin:-5px 0 12px;}.tabs{display:grid;grid-template-columns:repeat(auto-fit,minmax(135px,1fr));gap:10px;margin-bottom:15px;}.tab{display:block;background:#161616;border:1px solid #282828;border-radius:13px;padding:13px;text-decoration:none;color:#ddd;}.tab.active{background:#c00000;border-color:#ff2b2b;color:#fff;box-shadow:0 0 16px rgba(255,0,0,.18);}.tab-title{font-size:12px;font-weight:bold;margin-bottom:7px;}.tab-count{font-size:24px;font-weight:bold;color:#ff3c3c;}.tab.active .tab-count{color:#fff;}.section-title{font-size:18px;color:#ff2b2b;font-weight:bold;margin:8px 0 14px;}.order-card{background:#161616;border:1px solid #242424;border-radius:15px;margin-bottom:16px;overflow:hidden;box-shadow:0 0 18px rgba(255,0,0,.04);}.order-head{padding:14px;background:#111;border-bottom:1px solid #242424;display:flex;justify-content:space-between;gap:10px;align-items:flex-start;}.order-title{font-size:15px;color:#ff3c3c;font-weight:bold;line-height:1.4;}.order-id{font-size:12px;color:#999;margin-top:4px;line-height:1.5;}.badge{padding:6px 9px;border-radius:999px;font-size:11px;font-weight:bold;white-space:nowrap;text-align:center;}.badge.pending{background:#5c4500;color:#ffd35c;}.badge.paid{background:#005c2a;color:#7dffb4;}.badge.failed{background:#5c0000;color:#ff8a8a;}.order-body{padding:14px;}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px;margin-bottom:13px;}.item{background:#101010;border:1px solid #282828;border-radius:10px;padding:10px;}.label{font-size:11px;color:#888;margin-bottom:5px;}.value{font-size:13px;color:#fff;line-height:1.5;word-break:break-word;}.file-row{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;}.file-btn{display:inline-block;background:#c00000;color:#fff;text-decoration:none;border-radius:9px;padding:9px 11px;font-size:12px;font-weight:bold;border:none;cursor:pointer;}.file-btn.dark{background:#333;}.forms{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:10px;margin-top:12px;}.form-box{background:#101010;border:1px solid #282828;border-radius:12px;padding:12px;}select,input[type=file]{width:100%;background:#1b1b1b;border:1px solid #333;border-radius:9px;padding:10px;color:#fff;font-size:13px;margin-bottom:9px;}button{width:100%;background:#c00000;color:#fff;border:none;border-radius:9px;padding:10px;font-weight:bold;cursor:pointer;}button:hover,.file-btn:hover{background:#ff2b2b;}.delete-btn{background:#4b0000!important;}.delete-btn:hover{background:#ff0000!important;}.empty{text-align:center;color:#777;padding:50px 15px;}.price{color:#00ff7b;font-weight:bold;}.small-muted{font-size:11px;color:#888;margin-top:6px;line-height:1.5;}@media(max-width:600px){.order-head{flex-direction:column;}.badge{align-self:flex-start;}.header-title{font-size:15px;}.container{padding:10px;}}`;

  const rows = (orders.results || []).map(o => {
    const payStatus = o.payment_status || "pending";
    const payClass = payStatus === "paid" ? "paid" : (payStatus === "failed" ? "failed" : "pending");
    return `<div class="order-card"><div class="order-head"><div><div class="order-title">${esc(o.kendaraan || "-")} — ${esc(o.ecu_type || "-")}</div><div class="order-id">Order #${Number(o.id)} | ${esc(o.created_at || "-")}</div></div><div class="badge ${payClass}">${esc(String(payStatus).toUpperCase())} / ${esc(o.status || "-")}</div></div><div class="order-body"><div class="grid"><div class="item"><div class="label">Nama</div><div class="value">${esc(o.nama || "-")}</div></div><div class="item"><div class="label">WhatsApp</div><div class="value">${esc(o.whatsapp || "-")}</div></div><div class="item"><div class="label">Email Hasil</div><div class="value">${esc(o.email || "-")}</div></div><div class="item"><div class="label">Harga</div><div class="value price">${rupiah(o.price || 0)}</div></div><div class="item"><div class="label">Duitku Reference</div><div class="value">${esc(o.duitku_reference || "-")}</div></div><div class="item"><div class="label">Merchant Order ID</div><div class="value">${esc(o.merchant_order_id || "-")}</div></div></div><div class="item"><div class="label">Permintaan Setting</div><div class="value">${nl2brEsc(o.permintaan || "-")}</div></div><div class="file-row">${o.ecu_file ? `<a class="file-btn" href="/remap/download.php?id=${o.id}&type=original">⬇ Download File ECU</a>` : ""}${o.result_file ? `<a class="file-btn dark" href="/remap/download.php?id=${o.id}&type=result">✅ Download Hasil Remap</a>` : ""}${o.payment_url ? `<a class="file-btn" href="${esc(o.payment_url)}" target="_blank">💳 Link Bayar</a>` : ""}</div><div class="forms"><form method="POST" class="form-box"><input type="hidden" name="action" value="update_status"><input type="hidden" name="order_id" value="${o.id}"><select name="status"><option value="menunggu_pembayaran" ${o.status === "menunggu_pembayaran" ? "selected" : ""}>Menunggu Pembayaran</option><option value="diproses" ${o.status === "diproses" ? "selected" : ""}>Diproses</option><option value="selesai" ${o.status === "selesai" ? "selected" : ""}>Selesai</option><option value="ditolak" ${o.status === "ditolak" ? "selected" : ""}>Ditolak</option></select><button type="submit">Update Status</button><div class="small-muted">Ubah status manual kalau diperlukan.</div></form><form method="POST" enctype="multipart/form-data" class="form-box"><input type="hidden" name="action" value="upload_result"><input type="hidden" name="order_id" value="${o.id}"><input type="file" name="result_file" accept=".bin,.ori,.hex,.zip,.rar,.pdf,.txt" required><button type="submit">Upload Hasil Remap + Kirim Email</button><div class="small-muted">Status otomatis selesai dan email hasil dikirim ke customer.</div></form><form method="POST" class="form-box" onsubmit="return confirm('Yakin hapus order remap ini? Semua file dan notifikasi order ini ikut dihapus.');"><input type="hidden" name="action" value="delete"><input type="hidden" name="order_id" value="${o.id}"><button type="submit" class="delete-btn">Hapus Order</button><div class="small-muted">Hapus order dari database + file ECU + hasil remap.</div></form></div></div></div>`;
  }).join("");

  const body = `<div class="header"><div><div class="header-title">⚡ Admin Order Remap</div><div class="header-user">Login sebagai: ${esc(user.nama || "Admin")} / ${esc(user.role || "")}</div></div><a href="/dashboard/" class="nav-btn">Kembali</a></div><div class="container">${success ? `<div class="alert success">${esc(success)}</div>` : ""}${error ? `<div class="alert error">${esc(error)}</div>` : ""}<div class="admin-version">V19: kategori order + tombol hapus order + upload hasil remap + email Resend + R2 bucket only</div><div class="tabs">${tab("menunggu_pembayaran", "Menunggu Bayar")}${tab("diproses", "Diproses")}${tab("selesai", "Selesai")}${tab("ditolak", "Ditolak")}${tab("semua", "Semua")}</div><div class="section-title">${esc(filterTitleMap[filter] || "Order Remap")}</div>${rows || `<div class="empty">Belum ada order remap.</div>`}</div>`;
  return htmlResponse(pageWrap({ title: "Admin Order Remap - IDS ECU REPAIR", styles, body }));
}


async function handlePanduan() {
  const styles = `*{margin:0;padding:0;box-sizing:border-box;}body{background:linear-gradient(rgba(0,0,0,.92),rgba(0,0,0,.94)),url('/media/css/bg-ecu.png');font-family:Arial,sans-serif;color:#fff;overflow-x:hidden;}.header{position:sticky;top:0;z-index:99;background:#111;padding:10px 14px;border-bottom:2px solid #c00000;display:flex;justify-content:space-between;align-items:center;border:none!important;box-shadow:0 0 14px rgba(255,0,0,.45);}.header .logo{font-weight:bold;margin-left:14px;color:#ff2b2b;font-size:17px;letter-spacing:1px;}.back{background:#1d1d1d;border:1px solid #333;padding:7px 11px;border-radius:10px;text-decoration:none;color:#fff;font-size:13px;margin-right:10px;}.container{max-width:900px;margin:auto;padding:16px;}.banner{width:100%;height:220px;object-fit:cover;border-radius:18px;margin-bottom:20px;border:1px solid #222;box-shadow:0 0 20px rgba(255,0,0,.15);opacity:0;transform:scale(1.03);animation:bannerLoad .9s ease forwards;}.hero{text-align:center;margin-bottom:25px;}.hero h1{font-size:28px;color:#ff2b2b;margin-bottom:12px;}.hero p{font-size:14px;line-height:1.8;color:#bbb;max-width:760px;margin:auto;}.card{background:#161616;border:1px solid #242424;border-radius:18px;padding:18px;margin-bottom:18px;}.card h2{font-size:21px;color:#ff3c3c;margin-bottom:14px;}.card p{font-size:14px;line-height:1.8;color:#ccc;margin-bottom:10px;}.step{display:flex;gap:14px;margin-bottom:14px;background:#101010;border:1px solid #252525;padding:14px;border-radius:14px;}.num{min-width:38px;height:38px;border-radius:50%;background:#c00000;display:flex;align-items:center;justify-content:center;font-weight:bold;}.step-content h3{font-size:16px;margin-bottom:6px;color:#fff;}.step-content p{font-size:13px;line-height:1.7;color:#bbb;}.warning{background:#190909;border:1px solid #4a1515;}.warning ul{padding-left:20px;}.warning li{margin-bottom:10px;line-height:1.8;color:#ddd;}.safe-box{background:#101010;border:1px solid #252525;padding:14px;border-radius:14px;margin-top:12px;}.safe-box b{color:#00ff88;}.cta{position:sticky;bottom:0;padding:18px 0 12px;background:linear-gradient(to top,#0d0d0d 70%,transparent);}.btn{display:block;width:100%;max-width:500px;margin:auto;background:#c00000;padding:16px;border-radius:14px;text-align:center;text-decoration:none;color:#fff;font-weight:bold;font-size:15px;box-shadow:0 0 20px rgba(255,0,0,.25);}.btn:hover{background:#ff2b2b;}.footer{text-align:center;font-size:12px;color:#777;padding:18px;line-height:1.8;}@keyframes bannerLoad{0%{opacity:0;transform:scale(1.05);filter:blur(8px);}100%{opacity:1;transform:scale(1);filter:blur(0);}}@media(max-width:600px){.banner{height:150px;}.hero h1{font-size:22px;}.card{padding:15px;}.step{padding:12px;}}`;
  const body = `
<div class="header">
...
<div class="container">
<img src="/media/css/og-banner-read-write.png" class="banner">
...
<div class="container">
<section class="hero">
<img src="/media/css/og-remap-prosedur-order.png" class="banner" alt="Prosedur Remap ECU IDS Repair">
