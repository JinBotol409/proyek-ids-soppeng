...
async function handleAdminRemap(request, env) {
  ...
  const user = await requireLogin(request, env);
  if (!user) return redirectToLogin(request);
  if (user.role !== "admin") return new Response("Akses admin ditolak", { status: 403 });
  const url = new URL(request.url);
  ...
  if (request.method === "POST") {
    ...
    const order = await env.DB.prepare(`SELECT * FROM remap_orders WHERE id=?`).bind(orderId).first();
    if (!order) return Response.redirect(`${url.origin}/remap/admin/?error=${encodeURIComponent("Order tidak ditemukan")}`, 302);

    if (action === "delete") {
      if (order.ecu_file) { try { await getR2Bucket(env)?.delete(r2KeyFromPublicPath(order.ecu_file)); } catch (e) {} }
      if (order.result_file) { try { await getR2Bucket(env)?.delete(r2KeyFromPublicPath(order.result_file)); } catch (e) {} }
      await env.DB.prepare(`DELETE FROM remap_orders WHERE id=?`).bind(orderId).run();
      return Response.redirect(`${url.origin}/remap/admin/?success=${encodeURIComponent("Order remap dihapus")}`, 302);
    }
    ...
    if (action === "mark_paid") {
      ...
      try { await sendRemapPaidEmail(env, updated); } catch (e) {}
      return Response.redirect(`${url.origin}/remap/admin/?success=${encodeURIComponent("Order ditandai paid")}`, 302);
    }

    if (action === "upload_result") {
      const file = form.get("result_file");
      if (!file || !file.size) return Response.redirect(`${url.origin}/remap/admin/?error=${encodeURIComponent("File hasil belum dipilih")}`, 302);
      if (file.size > 150 * 1024 * 1024) return Response.redirect(`${url.origin}/remap/admin/?error=${encodeURIComponent("Ukuran file hasil maksimal 150MB")}`, 302);
      if (!isAllowedRemapResult(file)) return Response.redirect(`${url.origin}/remap/admin/?error=${encodeURIComponent("Format hasil harus BIN, ORI, HEX, ZIP, RAR, PDF, atau TXT")}`, 302);
      if (order.result_file) { try { await getR2Bucket(env)?.delete(r2KeyFromPublicPath(order.result_file)); } catch (e) {} }
      const resultKey = await uploadRemapFile(env, file, "remap_results", "result");
      await env.DB.prepare(`UPDATE remap_orders SET result_file=?, result_file_original=?, status='selesai', payment_status=CASE WHEN payment_status='paid' THEN payment_status ELSE 'paid' END, result_sent_at=? WHERE id=?`)
        .bind(resultKey, file.name || "hasil-remap.bin", new Date().toISOString(), orderId).run();
      const updated = await env.DB.prepare(`SELECT * FROM remap_orders WHERE id=?`).bind(orderId).first();
      await addRemapNotification(env, updated.user_id, updated.id, "File Remap Selesai", `File hasil remap ${updated.kendaraan} - ${updated.ecu_type} sudah tersedia.`);
      try { await sendRemapResultEmail(request, env, updated); } catch (e) {}
      return Response.redirect(`${url.origin}/remap/admin/?success=${encodeURIComponent("File hasil remap diupload dan email dikirim")}`, 302);
    }
  }
  ...
  const styles = `*{margin:0;padding:0;box-sizing:border-box;}body{background:#0d0d0d;color:#fff;font-family:Arial,sans-serif;}.header{position:sticky;top:0;z-index:99;background:#111;border-bottom:2px solid #c00000;padding:14px 15px;display:flex;align-items:center;justify-content:space-between;gap:10px;box-shadow:0 0 20px rgba(255,0,0,.12);}.header-title{font-size:16px;font-weight:bold;color:#ff2b2b;line-height:1.4;}.nav-btn{background:#1b1b1b;border:1px solid #333;border-radius:9px;padding:8px 12px;color:#fff;text-decoration:none;font-size:12px;white-space:nowrap;}.container{width:100%;max-width:1050px;margin:auto;padding:14px;}.alert{padding:11px;border-radius:10px;margin-bottom:12px;font-size:13px}.ok{background:#005c2a}.err{background:#9b0000}.admin-version{font-size:11px;color:#777;margin:-5px 0 12px;}.section-title{font-size:18px;color:#ff2b2b;font-weight:bold;margin:8px 0 14px;}.order-card{background:#161616;border:1px solid #242424;border-radius:15px;margin-bottom:16px;overflow:hidden;box-shadow:0 0 18px rgba(255,0,0,.04);}.order-head{padding:14px;background:#111;border-bottom:1px solid #242424;display:flex;justify-content:space-between;gap:10px;align-items:flex-start;}.order-title{font-size:15px;color:#ff3c3c;font-weight:bold;line-height:1.4;}.order-id{font-size:12px;color:#999;margin-top:4px;line-height:1.5;}.badge{padding:6px 9px;border-radius:999px;font-size:11px;font-weight:bold;white-space:nowrap;text-align:center;background:#5c4500;color:#ffd35c;}.order-body{padding:14px;}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px;margin-bottom:13px;}.item{background:#101010;border:1px solid #282828;border-radius:10px;padding:10px;}.label{font-size:11px;color:#888;margin-bottom:5px;}.value{font-size:13px;color:#fff;line-height:1.5;word-break:break-word;}.file-row{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;}.file-btn,.btn{display:inline-block;background:#c00000;color:#fff;text-decoration:none;border:none;border-radius:9px;padding:9px 11px;font-size:12px;font-weight:bold;cursor:pointer}.btn.dark{background:#333}.btn.green{background:#006b2d}.upload-form{margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;align-items:center}.upload-form input{background:#101010;border:1px solid #333;color:#fff;border-radius:8px;padding:8px;max-width:100%;}.empty{text-align:center;color:#777;padding:50px 15px;}`;
  const body = `<div class="header"><div class="header-title">⚡ Admin Order Remap</div><a href="/dashboard/" class="nav-btn">Kembali</a></div><div class="container">
${success ? `<div class="alert ok">${esc(success)}</div>` : ""}${error ? `<div class="alert err">${esc(error)}</div>` : ""}
<div class="admin-version">V18: order Duitku + upload hasil remap + email Resend + R2 bucket only</div><div class="section-title">Semua Order Remap</div>
${(orders.results || []).map(o => `<div class="order-card"><div class="order-head"><div><div class="order-title">${esc(o.kendaraan || "-")} — ${esc(o.ecu_type || "-")}</div><div class="order-id">Order #${o.id} | ${esc(o.created_at || "-")}<br>Merchant: ${esc(o.merchant_order_id || "-")}</div></div><div class="badge">${esc((o.payment_status || "pending").toUpperCase())} / ${esc(o.status || "-")}</div></div><div class="order-body"><div class="grid"><div class="item"><div class="label">Nama</div><div class="value">${esc(o.nama || "-")}</div></div><div class="item"><div class="label">WhatsApp</div><div class="value">${esc(o.whatsapp || "-")}</div></div><div class="item"><div class="label">Email Hasil</div><div class="value">${esc(o.email || "-")}</div></div><div class="item"><div class="label">Harga</div><div class="value">${rupiah(o.price || 0)}</div></div></div><div class="item"><div class="label">Permintaan Setting</div><div class="value">${nl2brEsc(o.permintaan || "-")}</div></div><div class="file-row">${o.ecu_file ? `<a class="file-btn" href="/remap/download.php?id=${o.id}&type=original">⬇ Download File ECU</a>` : ""}${o.result_file ? `<a class="file-btn" href="/remap/download.php?id=${o.id}&type=result">✅ Download Hasil Remap</a>` : ""}${o.payment_url ? `<a class="file-btn" href="${esc(o.payment_url)}" target="_blank">💳 Link Bayar</a>` : ""}</div><form method="POST" class="upload-form" enctype="multipart/form-data"><input type="hidden" name="action" value="upload_result"><input type="hidden" name="order_id" value="${o.id}"><input type="file" name="result_file" accept=".bin,.ori,.hex,.zip,.rar,.pdf,.txt" required><button class="btn green" type="submit">Upload & Kirim Email</button></form><div class="file-row"><form method="POST"><input type="hidden" name="action" value="mark_paid"><input type="hidden" name="order_id" value="${o.id}"><button class="btn dark" type="submit">Tandai Paid</button></form><form method="POST" onsubmit="return confirm('Hapus order dan file R2?')"><input type="hidden" name="action" value="delete"><input type="hidden" name="order_id" value="${o.id}"><button class="btn" type="submit">Hapus</button></form></div></div></div>`).join("") || `<div class="empty">Belum ada order remap.</div>`}</div>`;
