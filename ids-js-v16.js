...
async function deleteR2KeyIfAllowed(env, value = "") {
  const bucket = getR2Bucket(env);
  if (!bucket) return;
  const key = r2KeyFromPublicPath(value);
  if (!key) return;
  const allowed = ["profile/", "repairs/", "videos/", "ecu_files/", "og_images/", "gallery/", "fighter/", "remap_files/", "remap_results/"];
  if (!allowed.some(p => key.startsWith(p))) return;
...
async function deletePostFilesFromBucket(env, post) {
  ...
  for (const img of images) await deleteR2KeyIfAllowed(env, img);
  if (post.video) await deleteR2KeyIfAllowed(env, post.video);
  if (post.ecu_file) await deleteR2KeyIfAllowed(env, post.ecu_file);
  if (post.og_image) await deleteR2KeyIfAllowed(env, post.og_image);
}
