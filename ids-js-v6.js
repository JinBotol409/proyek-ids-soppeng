...
async function uploadProfilePhoto(env, userId, file) {
  ...
  const key = `profiles/user-${userId}-${Date.now()}-${randomToken(4)}.${ext}`;
  ...
  if (request.method === "POST") {
    try {
      ...
      if (foto && typeof foto === "object" && foto.size > 0) {
        fotoUrl = await uploadProfilePhoto(env, user.id, foto);
      }
...
<img src="${esc(shownUser?.foto_profil || '/css/default-profile.png')}">
<div class="edit-icon">✎</div>
</div>
<div class="form-group">
<label>Foto Profil</label>
<input type="file" name="foto" accept="image/*">
<div class="hint">Format JPG, PNG, WEBP, GIF. Maksimal 5MB.</div>
</div>
<div class="form-group">
