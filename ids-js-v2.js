...
const CDN = "https://cdn.idsrepair.com";
...
async function handleLogin(request, env) {
  ...
  if (!email || !password) {
    return htmlAuthMessage("Login gagal", "Email dan password wajib diisi.", 400);
  }
  ...
  if (!user) {
    return htmlAuthMessage("Login gagal", "Email atau password salah.", 401);
  }

  const ok = await verifyPassword(password, user.password);
  if (!ok) {
    return htmlAuthMessage("Login gagal", "Email atau password salah. Jika akun lama masih memakai hash PHP $2y$, migrasi/reset password dulu ke format Worker.", 401);
  }

  if (Number(user.is_verified || 0) === 0) {
    return htmlAuthMessage("Akun belum diverifikasi", "Akun Anda belum diverifikasi. Silakan verifikasi akun atau ubah is_verified menjadi 1 di D1.", 403);
  }
...
async function handleRegister(request, env) {
  ...
  if (!nama || !email || !password || !confirm) {
    return htmlAuthMessage("Register gagal", "Semua data wajib diisi.", 400);
  }
  if (!/^[a-zA-Z0-9 ]+$/.test(nama)) {
    return htmlAuthMessage("Register gagal", "Nama hanya boleh mengandung huruf, angka, dan spasi.", 400);
  }
  if (password !== confirm) {
    return htmlAuthMessage("Register gagal", "Password dan konfirmasi password tidak sama.", 400);
  }
  if (password.length < 6 || !/[A-Z]/.test(password) || !/[0-9]/.test(password)) {
    return htmlAuthMessage("Register gagal", "Password minimal 6 karakter, wajib ada huruf besar dan angka.", 400);
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    return htmlAuthMessage("Register gagal", "Format email tidak valid.", 400);
  }

  const namaExist = await env.DB.prepare("SELECT id FROM tamu WHERE nama=? LIMIT 1").bind(nama).first();
  if (namaExist) return htmlAuthMessage("Register gagal", "Nama lengkap sudah digunakan.", 409);

  const emailExist = await env.DB.prepare("SELECT id FROM tamu WHERE lower(email)=? LIMIT 1").bind(email).first();
  if (emailExist) return htmlAuthMessage("Register gagal", "Email sudah terdaftar.", 409);

  const passHash = await hashPassword(password);
  const tokenVerify = randomToken(16);
  ...
  ).bind(nama, email, passHash, "tamu", 0, tokenVerify, expired, 1).run();

  const user = await env.DB.prepare("SELECT id,nama,email,role FROM tamu WHERE lower(email)=? LIMIT 1").bind(email).first();
  const sessionToken = await createSession(env, user.id);

  const headers = new Headers();
  headers.set("Location", "/");
  headers.append("Set-Cookie", cookieHeader(sessionToken));
  return new Response(null, { status: 302, headers });
}
...
async function handleForgotPassword(request, env) {
  ...
  const form = await request.formData();
  const email = String(form.get("email") || "").trim().toLowerCase();
  if (!email) return htmlAuthMessage("Reset password", "Email wajib diisi.", 400);

  const user = await env.DB.prepare("SELECT id,nama,email FROM tamu WHERE lower(email)=? LIMIT 1").bind(email).first();
  if (!user) return htmlAuthMessage("Reset password", "Email tidak ditemukan.", 404);

  const token = randomToken(16);
  const expired = new Date(Date.now() + 20 * 60 * 1000).toISOString();
  await env.DB.prepare("UPDATE tamu SET otp=?, otp_expired=? WHERE id=?").bind(token, expired, user.id).run();

  const resetUrl = `/auth/reset-password.php?token=${encodeURIComponent(token)}&email=${encodeURIComponent(email)}`;
  return htmlAuthMessage("Reset password", `Link reset dibuat: ${resetUrl}`);
}
...
async function handleVerify(request, env) {
  ...
  if (Number(user.is_verified || 0) === 1) return htmlAuthMessage("Akun sudah aktif", "Akun sudah terverifikasi.");
  if (user.otp_expired && new Date(user.otp_expired).getTime() < Date.now()) {
    ...
    return htmlAuthMessage("Verifikasi gagal", "Akun dihapus karena link verifikasi sudah expired.", 400);
  }
...
export default {
  async fetch(request, env) {
    ...
    try {
      if (path === "/" || path === "/index.php") return handleHome(request, env);
      if (path === "/posts/repair/" || path === "/posts/repair" || path === "/posts/repair/index.php") return handleRepair(request, env);
      if (path === "/posts/view_repair/" || path === "/posts/view_repair" || path === "/posts/view_repair/index.php") return handleViewRepair(request, env);
      if (path === "/posts/ecu_files/" || path === "/posts/ecu_files" || path === "/posts/ecu_files/index.php") return handleEcuFiles(request, env);
      if (path === "/posts/download_ecu.php" || path === "/posts/download_ecu/" || path === "/posts/download_ecu") return handleDownloadEcu(request, env);
      if (path === "/posts/create_repair/" || path === "/posts/create_repair" || path === "/posts/create_repair/index.php") return handleCreateRepair(request, env);
      ...
      if (path === "/sport/" || path === "/sport" || path === "/sport/index.php") return handleSport();
      if (path === "/sport/fighter-awaludin/" || path === "/sport/fighter-awaludin" || path === "/sport/fighter-awaludin/index.php") return handleFighter();
      if (path === "/ai-ecu/" || path === "/ai-ecu" || path === "/ai-ecu/index.php") return handleAiEcu();
      if (path === "/ai-ecu/ai_reply.php") return handleAiReply(request);
      if (path === "/remap/" || path === "/remap" || path === "/remap/index.php") return handleRemap(request);
      if (path === "/remap/admin/" || path === "/remap/admin" || path === "/remap/admin/index.php") return handleAdminRemap(request, env);
      if (path === "/panduan-read-and-write-ecu/" || path === "/panduan-read-and-write-ecu" || path === "/panduan-read-and-write-ecu/index.php") return handlePanduan();
      if (path === "/panduan-read-and-write-ecu/remap-prosedur-order/" || path === "/panduan-read-and-write-ecu/remap-prosedur-order" || path === "/panduan-read-and-write-ecu/remap-prosedur-order/index.php") return handleProsedur();
...
function mediaCloudUrl(value, folder) {
  ...
  return `${CDN}/${folder}/${value.replace(/^\/+/, "")}`;
}
...
`).all();
  ...
  const postsHtml = (latest.results || []).map(p => {
    ...
    for (const img of images) {
      const imgSrc = /^https?:\/\//i.test(img) ? img : `${CDN}/repairs/${img.replace(/^\/+/, "")}`;
    ...
    if (p.video) {
      const v = /^https?:\/\//i.test(p.video.trim()) ? p.video.trim() : `${CDN}/videos/${p.video.trim().replace(/^\/+/, "")}`;
...
<div class="post-card">
  ...
  <div class="post-content">
    ...
    ${fileBox}
    <div class="post-info">
      <img class="author-img" src="${esc(p.foto_profil || "/css/default-profile.png")}" loading="lazy" alt="Author Profile">
      <div>
...
<img src="https://cdn.idsrepair.com/gallery/banner-trusted-ai-ready.png" alt="IDS ECU Repair teruji terpercaya SEO Google dan AI Agent Ready" loading="lazy" class="trust-banner-img">
</div>
</section>
...
<video controls><source src="https://cdn.idsrepair.com/gallery/video1.mp4" type="video/mp4"></video>
</div>
</div>
...
${ecuFile}
<div class="info">
<img src="${esc(row.foto_profil || "/css/default-profile.png")}" loading="lazy" class="author-img" alt="Author">
<div>
...
@media(max-width:600px){.logo{font-size:15px;}.nav-btn{font-size:11px;padding:7px 10px;}.container{padding:10px;}.title{font-size:16px;}.desc{font-size:13px;}.post-img{max-height:240px;}.media-slider .post-img,.media-slider .post-video{height:260px;}}
`;
...
<img src="${esc(c.foto_profil || "/css/default-profile.png")}" class="comment-avatar">
<div>
...
${currentUser ? `<a href="/posts/download_ecu.php?id=${post.id}" class="ecu-download-btn">⬇ Download File</a>` : `<a href="/index.php" class="ecu-lock-btn" onclick="alert('Login dulu untuk download file ECU');return false;">🔒 Login untuk Download</a>`}
</div>` : "";
...
<div class="post-info">
<img src="${esc(post.foto_profil || "/css/default-profile.png")}" class="author-img" alt="Foto profil">
<div>
...
${commentsHtml}
<a href="/index.php" class="login-link">Login untuk komentar</a>
</div>
...
${currentUser ? `<a href="/posts/download_ecu.php?id=${row.id}" class="download-btn">⬇</a>` : `<a href="/index.php" class="download-btn" style="background:#333;color:#aaa;" onclick="alert('Silakan login dulu untuk download file ECU');return false;">🔒</a>`}
</div>
</div>`;
  }).join("");
...
async function handleDownloadEcu(request, env) {
  ...
  const fileUrl = /^https?:\/\//i.test(fileValue) ? fileValue : `${CDN}/ecu_files/${fileValue.replace(/^\/+/, "")}`;
...
<div class="comment-item">
<div class="comment-top">
<img src="${esc(c.foto_profil || '/css/default-profile.png')}">
<div>
...
<img src="${esc(user?.foto_profil || '../css/default-profile.png')}" class="profile-img">
...
<img src="${esc(user?.foto_profil || '../css/default-profile.png')}" class="dropdown-img">
...
<img src="${esc(user?.foto_profil || '../css/default-profile.png')}">
<div>
<h1>Panel Teknisi</h1>
...
<div class="stat-card"><h2>0</h2><p>Order ECU Saya</p></div>
</div>
...
msgDiv.style.color='lime';msgDiv.innerText='Password berhasil diubah';
}`;
...
${(posts.results || []).map(p => {
  ...
  const yt = getYoutubeId(p.youtube_url || "");
  const slides = [
    ...images.map(img => `<div class="slide"><a href="/posts/view_repair/?id=${p.id}"><img class="post-img" src="${esc(img)}"></a></div>`),
    ...(p.video ? [`<div class="slide"><video class="post-video" controls playsinline preload="metadata"><source src="${esc(p.video)}" type="video/mp4"></video></div>`] : []),
...
async function handleChangePassword(request, env) {
  ...
  if (!ok) {
    return jsonResponse({ success: false, message: "Password lama salah atau masih format bcrypt PHP." }, 401);
  }
