...
const PUBLIC_FILE_ROUTE = "/bucket";
...
async function handleBucketFile(request, env) {
  const url = new URL(request.url);
  let key = decodeURIComponent(url.pathname.replace(/^\/bucket\/?/, "")).replace(/^\/+/, "");
  ...
  if (key.startsWith("ecu_files/")) {
    ...
    return new Response("Folder bucket tidak diizinkan", { status: 403 });
  }

  const bucket = getR2Bucket(env);
  if (!bucket) return new Response("Binding bucket belum ditemukan", { status: 500 });
