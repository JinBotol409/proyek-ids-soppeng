...
function makeSessionValue(user) {
  const raw = `${user.id}|${user.email}|${user.role || "tamu"}`;
  return btoa(raw);
}
...
function readSessionValue(value = "") {
  try {
    ...
    const [id, email, role] = raw.split("|");
    if (!id || !email) return null;
    return { id: Number(id), email, role: role || "tamu" };
  } catch (e) {
    return null;
  }
}
...
async function getCurrentUser(request, env) {
  ...
  const session = readSessionValue(cookies.ids_session || "");
  if (!session || !session.id) return null;

  const user = await env.DB.prepare(
    "SELECT id,nama,email,role,poin,foto_profil,is_verified FROM tamu WHERE id=? LIMIT 1"
...
async function handleLogin(request, env) {
  ...
  if (!email || !password) {
    return htmlResponse(pageWrap({
      title: "Login Gagal",
      body: `<div style="padding:25px;color:white;background:#111;min-height:100vh;font-family:Arial">
<h2>Login gagal</h2>
<p>Email dan password wajib diisi.</p>
<a href="/" style="color:#ff3c3c">Kembali</a>
</div>`
    }), 400);
  }
...
<a href="/" style="color:#ff3c3c">Kembali</a>
  ...
  if (!ok) {
    return htmlResponse(pageWrap({
      ...
      body: `<div style="padding:25px;color:white;background:#111;min-height:100vh;font-family:Arial">
<h2>Login gagal</h2>
<p>Password di database masih format bcrypt PHP. Cloudflare Worker editor tidak bisa membaca bcrypt tanpa library build.</p>
<p>Solusi: reset password akun ini ke format Worker SHA melalui halaman register baru atau query update password.</p>
<a href="/" style="color:#ff3c3c">Kembali</a>
</div>`
    }), 401);
  }
  ...
  headers.append("Set-Cookie", `ids_session=${encodeURIComponent(makeSessionValue(user))}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=604800`);
...
async function handleLogout(request, env) {
  ...
  headers.append("Set-Cookie", "ids_session=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0");
  return new Response(null, { status: 302, headers });
}
...
async function handleRegister(request, env) {
  ...
  if (!nama || !email || !password || password !== confirm) {
    return htmlResponse(pageWrap({
      title: "Register Gagal",
      body: `<div style="padding:25px;color:white;background:#111;min-height:100vh;font-family:Arial">
<h2>Register gagal</h2>
<p>Data belum lengkap atau konfirmasi password tidak cocok.</p>
<a href="/" style="color:#ff3c3c">Kembali</a>
</div>`
    }), 400);
  }

  const exists = await env.DB.prepare("SELECT id FROM tamu WHERE lower(email)=? LIMIT 1").bind(email).first();
  if (exists) {
    return htmlResponse(pageWrap({
      title: "Register Gagal",
      body: `<div style="padding:25px;color:white;background:#111;min-height:100vh;font-family:Arial">
...
<a href="/" style="color:#ff3c3c">Kembali</a>
  ...
  await env.DB.prepare(
    "INSERT INTO tamu (nama,email,password,role,poin,is_verified) VALUES (?,?,?,?,?,?)"
  ).bind(nama, email, passHash, "tamu", 0, 1).run();

  const user = await env.DB.prepare(
    "SELECT id,nama,email,role FROM tamu WHERE lower(email)=? LIMIT 1"
  ).bind(email).first();
...
async function handleForgotPassword(request, env) {
  return htmlResponse(pageWrap({
    ...
    body: `<div style="padding:25px;color:white;background:#111;min-height:100vh;font-family:Arial">
<h2>Reset password belum diaktifkan</h2>
<p>Untuk sementara reset password dilakukan dari admin database.</p>
<a href="/" style="color:#ff3c3c">Kembali</a>
</div>`
  }));
}
...
<div class="ecu-file-box">
  ...
  <a href="/" onclick="openLoginModal();return false;" class="ecu-lock-btn">🔒</a>
</div>` : "";
...
<a href="#" onclick="openLoginModal();closeMenu();return false;">🔐 Login</a>
...
<a href="/" class="ecu-download-btn" style="background:#333;color:#aaa;" onclick="alert('Login dulu untuk download file ECU');return false;">🔒</a>
</div>` : "";
...
<a href="/index.php" class="ecu-lock-btn" onclick="alert('Login dulu untuk download file ECU');return false;">🔒 Login untuk Download</a>
</div>` : "";
...
<a href="/index.php" class="download-btn" style="background:#333;color:#aaa;" onclick="alert('Silakan login dulu untuk download file ECU');return false;">🔒</a>
</div>
</div>`;
  }).join("");
